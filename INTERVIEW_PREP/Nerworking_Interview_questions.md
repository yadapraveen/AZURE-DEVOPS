...
- ASGs are applied to VMs and are used in conjunction with NSGs. By associating an ASG tag with a network security rule, you can define rules that apply to a group of VMs sharing the same tag. ASGs simplify the management of security rules in a multi-tier application by grouping VMs that belong to the same application tier. This makes it easier to apply and manage security policies for a specific application.


- Is there is two subnets and in one subnet there is login app. logout app payment , catalog etc  and in sunet is db, now login and logout app should not access the db . SO, now here we can do 2 thing 1 is adding all ip addresses in inound rules. if there is many ipadfresses i i mean applications, Now asg comes into picture 
...
- In a vnet , there are db, app, firewall and xzy subnets, now xyz should not access db subnet. The priority of this rule is 65000, so we need to create a Deny rule with less than 65000 priority number.
...
**Firewall**: Designed for controlling both outbound and inbound traffic to and from resources within a Virtual Network (VNet).

**NSG**: Typically associated with subnets or individual network interfaces to control traffic within a VNet and between VNets.
...
Operates at Layer 7 (Application layer) of the OSI model. Provides advanced application-level routing, SSL termination, and web application firewall (WAF) capabilities. Suited for distributing traffic based on application awareness.

### Azure Load Balancer:
Operates at Layer 4 (Transport layer) of the OSI model. Distributes network traffic based on IP address and port. Suitable for generic TCP/UDP load balancing without application-specific features.

![alt text](image-2.png)

- Here there is a 3 tier application it consists of website, application, database. website, application are having 2 copies.
...
- I will be firstly configure firewal. In firewall i will add NAT Rule
